...
class Attention(tk.layers.Layer):
    ...
    def call(self,x):
        ...
        print("The shape of the query is ",query.shape)
        ...
        print("The shpa eof the y is ",y.shape)
        print("The height is {} and the width is {}".format(height,width))
        y = tf.reshape(y,(-1,height,width,self.output_channel))
        print("The sdhap eof the y is ",y.shape)
        return y
...
class AttentionStem(tk.layers.Layer):
    ...
    def call(self,x):
        ...
        #print("THe type of the elements in tyhe value_out is",type(value[1]))
        #value_out = tf.image.extract_patches(images = value[1], sizes = [1,self.kernel_size,self.kernel_size,1], strides = [1,1,1,1], rates = [1,1,1,1],padding = "VALID")    
        print("THe sdhap eof the value_out is ",value_out.shape)
        print()
        #value_out.append(tf.image.extract_patches(images = value[1], sizes = [1,self.kernel_size,self.kernel_size,1], strides = [1,1,1,1], rates = [1,1,1,1],padding = "VALID")) 
        #value_out.append(tf.image.extract_patches(images = value[2], sizes = [1,self.kernel_size,self.kernel_size,1], strides = [1,1,1,1], rates = [1,1,1,1],padding = "VALID"))
        #value_out.append(tf.image.extract_patches(images = value[3], sizes = [1,self.kernel_size,self.kernel_size,1], strides = [1,1,1,1], rates = [1,1,1,1],padding = "VALID")) 
        #value_out = tf.keras.backend.stack(value_out)
        #value_out = tk.backend.stack()
